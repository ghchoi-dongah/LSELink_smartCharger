...
public class CRC16 {
    ...
    final static private int[] tableMODBUS = {
    ...
    static public short setCrc16(byte[] bytes, int offset, int length) {
        short crc = 0x00;
        for (int i = offset; i < length; i++) {
            crc = (short) ((crc << 8) ^ table[((crc >> 8) ^ bytes[i + 1]) & 0xff]);
        }
        return crc;
    }

    static public byte[] setCrc16ModBus(byte[] bytes, int offset, int length) {
        int idx = 0x00;
        int crcFull = 0xffff;
        ...
        resultCRC[0] = (byte) ((crcFull >> 8) & 0xff);
